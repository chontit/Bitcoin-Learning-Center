...
<head>
    ...
    <title>Short Position Calculator</title>
	<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23ef4444' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpolyline points='23 18 13.5 8.5 8.5 13.5 1 6'%3E%3C/polyline%3E%3Cpolyline points='17 18 23 18 23 12'%3E%3C/polyline%3E%3C/svg%3E" type="image/svg+xml">
    ...
    <style>
        ...
        ::-webkit-scrollbar-thumb { background: #7f1d1d; border-radius: 4px; } /* Red Thumb for Short */
...
<body class="bg-gray-900 text-gray-200 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <header class="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-gray-700 pb-4 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-red-500"><i class="fa-solid fa-arrow-trend-down"></i> Short Position Sizing</span>
			<a href="index.php" class="text-gray-600 hover:text-white transition-colors text-xl" title="กลับหน้าหลัก">
				<i class="fa-solid fa-house"></i>
			</a>
			</h1>
                <p class="text-sm text-gray-400 mt-1">เครื่องมือคำนวณไม้เทรด (ขาลง)</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
                    <h2 class="text-xl font-semibold mb-4 text-white border-l-4 border-red-500 pl-3">1. Portfolio Setting</h2>
                ...
                <div class="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
                    <h2 class="text-xl font-semibold mb-4 text-white border-l-4 border-orange-500 pl-3">2. Trade Setup (Short)</h2>
                    ...
                    <div class="space-y-4">
                        <div>
                            <div class="flex justify-between">
                                <label class="text-sm text-gray-300">ราคาเข้า (Short Entry USD)</label>
                        ...
                        <div>
                            <div class="flex justify-between items-center">
                                <label class="text-sm text-red-300">ราคา Stop Loss (สูงกว่า)</label>
                        ...
                        <div>
                            <div class="flex justify-between items-center">
                                <label class="text-sm text-green-300">ราคา Take Profit (ต่ำกว่า)</label>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
                    <h2 class="text-xl font-semibold mb-6 text-white border-l-4 border-red-500 pl-3">Results (Short)</h2>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-gradient-to-br from-gray-800 to-red-900/20 p-6 rounded-lg border border-red-900/50 relative overflow-hidden">
                            <div class="absolute top-0 right-0 p-4 opacity-10">
                                <i class="fa-solid fa-money-bill-wave text-6xl text-red-500"></i>
                            </div>
                            <h3 class="text-gray-400 text-sm uppercase tracking-wider">Short Position Size</h3>
                    ...
                    <button onclick="saveJournal()" class="w-full mt-6 bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-lg transition duration-200 shadow-lg flex justify-center items-center gap-2">
                        <i class="fa-solid fa-save"></i> บันทึก Short เข้า Journal
                    </button>
                </div>
    ...
    <script>
        ...
        function adjustValue(id, step) {
            ...
            if(el.classList.contains('js-format-number')) el.value = formatWithCommas(val);
            else el.value = val;
        ...
        function syncRisk(source) {
            ...
            if (source === 'amount' || source === 'port') {
                ...
                if(portSize > 0) document.getElementById('risk-percent').value = ((amount / portSize) * 100).toFixed(2);
            } 
            if (source === 'percent') {
        ...
        function autoFillSLTP(price) {
            // SHORT LOGIC: SL Higher (+10%), TP Lower (-10%)
            const sl = price * 1.10;
            const tp = price * 0.90;
        ...
        function calculate() {
            ...
            // Calculate Percents
            if(entryUSD > 0) {
                // For Short: Loss is when SL > Entry (Positive diff in pure math, but represents loss zone)
                // We show Diff % from Entry
                if(slUSD > 0) {
                    const slDiff = ((slUSD - entryUSD) / entryUSD) * 100;
                    document.getElementById('sl-diff-percent').innerText = `${slDiff > 0 ? '+' : ''}${slDiff.toFixed(2)}%`;
                    // Color logic: High SL in Short is Bad? No, SL is always bad if hit. 
                    // But here we just show distance.
                    document.getElementById('sl-diff-percent').className = "text-xs font-bold text-red-400 bg-red-900/30 px-2 py-0.5 rounded";
                }
                if(tpUSD > 0) {
                    const tpDiff = ((tpUSD - entryUSD) / entryUSD) * 100;
                    document.getElementById('tp-diff-percent').innerText = `${tpDiff > 0 ? '+' : ''}${tpDiff.toFixed(2)}%`;
                    document.getElementById('tp-diff-percent').className = "text-xs font-bold text-green-400 bg-green-900/30 px-2 py-0.5 rounded";
                }
            }

            if (entryUSD === 0 || slUSD === 0 || riskAmount === 0) return;

            // SHORT LOGIC: Risk Distance is absolute difference
        ...
        function saveJournal() {
            ...
            const data = {
                ...
                type: 'Short' // Force Short for this file
            };
