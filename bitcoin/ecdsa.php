    ...
    <div class="overflow-y-auto p-5" style="max-height:78vh;">
      ...
      <div class="guide-step mb-4">
        ...
        <div class="space-y-2">
		  ...
		  <div class="flex items-center gap-3 info-card">
            <span class="inline-block w-3 h-3 rounded-full shrink-0" style="background:#ffd700;box-shadow:0 0 6px #ffd700;"></span>
            <div class="text-xs text-gray-300"><b class="text-[#ffd700]">สีเหลือง -(P1+P2)</b> — จุดตัดจุดที่ 3 บนเส้นโค้ง (ก่อนทำการสะท้อนค่า y)</div>
          </div>
		  <div class="flex items-center gap-3 info-card">
            <div class="w-8 border-t-2 border-dashed border-[#ff0055] shrink-0"></div>
            <div class="text-xs text-gray-300"><b class="text-[#ff0055]">เส้นประแดง (Laser Ray)</b> — การเดินทางของสมการเส้นตรงที่ "ชิ่งขอบ" (Wrap-around) จาก P1 ผ่าน P2 ไปจบที่จุดที่สาม</div>
          </div>
          <div class="flex items-center gap-3 info-card">
            <div class="w-8 border-t-2 border-dashed border-[#00ff41] shrink-0"></div>
            <div class="text-xs text-gray-300"><b class="text-[#00ff41]">เส้นประเขียว (Reflection)</b> — การสะท้อนจุดกลับ (Flip y) จากจุดที่ 3 ลงมาเป็นผลลัพธ์ R</div>
          </div>
        </div>
      </div>
      ...
      <div class="guide-step mb-4" style="border-color:rgba(255, 0, 85, 0.3)">
        <div class="text-white font-bold text-sm mb-3"><i class="fa-solid fa-route text-[#ff0055] mr-2"></i>ทำไมเส้นถึงวาร์ป? (Wrap-around Logic)</div>
        <div class="analogy-box">
          <div class="text-xs text-gray-300 leading-relaxed">
            ใน <b>Finite Field</b> พื้นที่กราฟจะเชื่อมต่อกันเหมือนด่านในเกม Pac-Man:
            <ul class="list-disc ml-4 mt-2 space-y-1">
              <li>เมื่อเส้นประวิ่ง <b class="text-white">ทะลุขอบบน</b> มันจะวาร์ปไปโผล่ <b class="text-white">ขอบล่าง</b> ทันที</li>
              <li>นี่คือพฤติกรรมของ <b>Modulo (mod p)</b> ที่ตบตัวเลขให้วนกลับมาอยู่ในขอบเขตเสมอ</li>
              <li>เส้นสีแดงที่ดูเหมือนหลายเส้น จริงๆ คือ <b class="text-[#ff0055]">"เส้นตรงเส้นเดียว"</b> ที่ถูกพับไปมาจนกว่าจะเจอจุดหมายครับ</li>
            </ul>
          </div>
        </div>
      </div>
...
if (p1.x===p2.x && p1.y===p2.y) {
  ...
  return { pt:{x:x3,y:y3}, steps:steps, lam:lam };
}
...
// --- FF Canvas Animation Loop ---
let ffAnimFrame = null;
let ffDashOffset = 0;

function renderFFLoop() {
  ffDashOffset -= 0.5; // ความเร็วของเส้นประ
  drawFFOverlay();     // สั่งวาดใหม่
  ffAnimFrame = requestAnimationFrame(renderFFLoop);
}

function initFFLab() {
  computeFFPoints(); updateFFLabel(); renderFFGrid(); drawFFOverlay();
  if (!ffAnimFrame) renderFFLoop(); // เริ่ม Animation
}
...
function drawFFOverlay() {
  ...
  // result point
  var resData=null, resLam=null;
  if(ffP1&&ffP2){ var r=ffAdd(ffP1,ffP2,ffPrime); if(r&&r.pt){ resData=r.pt; resLam=r.lam; } }
  // all curve points
  ffPoints.forEach(function(pt){
...
// --- ระบบวาดเส้นทะลุขอบ (Wrap-around Line) ---
  if(resData && ffP1 && ffP2 && resLam !== null) {
    var rx = PAD + resData.x * cw, ry = H - PAD - resData.y * ch; // พิกัด R บนจอ
    var rNegY = (p - resData.y) % p; // ค่า y ของจุดก่อนสะท้อน -(P1+P2)
    var rNegX_px = rx;
    var rNegY_px = H - PAD - rNegY * ch;
    var lam = resLam;
    var c0 = ((ffP1.y - lam * ffP1.x) % p + p) % p; // จุดตัดแกน y แรกสุด

    cx.save();
    
    // 1. วาดเส้นเลเซอร์จาก P1 -> ทะลุ P2 -> ไปจบที่ -(P1+P2)
    cx.strokeStyle = 'rgba(255, 0, 85, 0.6)'; // ปรับสีให้สว่างขึ้นนิดนึง
    cx.lineWidth = 1.5;
    cx.setLineDash([5, 5]);
    cx.lineDashOffset = typeof ffDashOffset !== 'undefined' ? ffDashOffset : 0;

    // คำนวณระยะทางแกน X ที่ต้องเดินทางจาก P1 ไป P2 และไปจุดจบ
    var dx12 = (ffP1.x === ffP2.x && ffP1.y === ffP2.y) ? 0 : (ffP2.x - ffP1.x + p) % p;
    var dx23 = (resData.x - ffP2.x + p) % p;
    var dx_total = dx12 + dx23;

    var t = 0;
    while (t < dx_total) {
      var curX = ffP1.x + t;
      var curY = ffP1.y + lam * t;
      
      // หาขอบแกน X และ Y ถัดไปที่เส้นจะวิ่งไปชน
      var nextX = Math.floor(curX / p + 1) * p;
      var dt_x = nextX - curX;
      
      var dt_y = Infinity;
      if (lam > 0) {
        var nextY = Math.floor(curY / p + 1) * p;
        dt_y = (nextY - curY) / lam;
      }
      
      // เลือกระยะที่สั้นที่สุดก่อนชนขอบ
      var dt = Math.min(dt_x, dt_y, dx_total - t);
      if (dt <= 0.0001) { t += 0.0001; continue; } // กันบั๊กค้างขอบจอ
      
      var x1 = curX % p;
      var y1 = curY % p;
      var x2 = (curX + dt) % p;
      var y2 = (curY + lam * dt) % p;
      
      // เชื่อมเส้นให้ติดขอบพอดี
      if (Math.abs(dt - dt_x) < 1e-6 && x2 < 1e-6) x2 = p;
      if (lam > 0 && Math.abs(dt - dt_y) < 1e-6 && y2 < 1e-6) y2 = p;

      cx.beginPath();
      cx.moveTo(PAD + x1 * cw, H - PAD - y1 * ch);
      cx.lineTo(PAD + x2 * cw, H - PAD - y2 * ch);
      cx.stroke();
    ...
    // 2. วาดจุด -(P1+P2) สีเหลืองส้ม
    cx.shadowBlur = 10; cx.shadowColor = 'rgba(255, 215, 0, 0.8)';
    cx.fillStyle = 'rgba(255, 215, 0, 0.9)';
    cx.beginPath();
    cx.arc(rNegX_px, rNegY_px, Math.min(cw, ch) * 0.38, 0, Math.PI * 2);
    cx.fill();
    cx.shadowBlur = 0;
    cx.fillStyle = 'white'; cx.font = 'bold 9px Share Tech Mono';
    cx.fillText('-(P1+P2)', rNegX_px + 8, rNegY_px - 2);

    // 3. วาดเส้นสะท้อนกลับลงมา (สีเขียว)
    cx.strokeStyle = 'rgba(0, 255, 65, 0.8)';
    cx.setLineDash([4, 4]);
    cx.lineDashOffset = typeof ffDashOffset !== 'undefined' ? ffDashOffset : 0;
    cx.beginPath();
    cx.moveTo(rNegX_px, rNegY_px);
    cx.lineTo(rx, ry);
    cx.stroke();
